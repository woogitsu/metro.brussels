...
| `tools/track/validate.py` | validate | 2 | 2 | 0 | 36 | 40 | `b5c2eb9` dołożył 1 mutację, zabitą. **37 → 40 (09.09.2026, 6.D69)**: opcja `--package` przestała być pusta i dołożyła trzy porównania — identyfikator pakietu z osi wobec żądanego, obecność pakietu w rejestrze i zgodność trzech pól deklaracji z `build_packages`. Kolumna ocalałych **nieprzeliczona** i taka zostaje: 2 / 2 to pomiar z datą |
| `tools/track/crosscheck_alignment.py` | validate | 2 | 2 | 0 | 19 | 31 | **6.B52 dołożył 11 mutacji** (19 → 30): droga zapasowa przez `api.openstreetmap.org` — siatka kafli, czytnik `/api/0.6/map`, tryb `--offline`. Kolumna ocalałych **nieprzeliczona** i taka zostaje: 2 / 2 to pomiar z datą, którego dopisanie kodu nie unieważnia. Przebieg na `7f116a4` (czyste drzewo, `operator,prog`): **22 zabite, 8 ocalałych, 0 nieuruchomionych** — z ośmiu dwie były prawdziwymi dziurami i zostały zamknięte, patrz `reports/osm-api-droga-zapasowa.md` §6. **30 → 31 (09.09.2026, 6.D62)**: wspólna pamięć kafli dołożyła JEDNO porównanie — `if origin == "osm-api" and licznik.pobrane > 1`, czyli warunek przerwy między kaflami, który po zmianie obowiązuje wyłącznie po wyjściu do sieci. Kolumna ocalałych **nieprzeliczona** i taka zostaje, z tego samego powodu co wiersz wyżej |
