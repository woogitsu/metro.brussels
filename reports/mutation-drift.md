...
| `tools/track/crosscheck_alignment.py` | validate | 2 | 2 | 0 | 19 | 30 | **6.B52 dołożył 11 mutacji** (19 → 30): droga zapasowa przez `api.openstreetmap.org` — siatka kafli, czytnik `/api/0.6/map`, tryb `--offline`. Kolumna ocalałych **nieprzeliczona** i taka zostaje: 2 / 2 to pomiar z datą, którego dopisanie kodu nie unieważnia. Przebieg na `7f116a4` (czyste drzewo, `operator,prog`): **22 zabite, 8 ocalałych, 0 nieuruchomionych** — z ośmiu dwie były prawdziwymi dziurami i zostały zamknięte, patrz `reports/osm-api-droga-zapasowa.md` §6 |
