...
| `tools/ci/assert_shot_metadata.py` | png-metadata | 2 | 32 | +30 | 33 | 108 | `a643f05` +4 ocalałe, `2c916de` +26 ocalałych (bramka peronu bez testów); 73 → 108 po dodaniu kontroli wizualnego przedłużenia, ocalałych nie przeliczono |
...
| `tools/track/crosscheck_alignment.py` | validate | 2 | 2 | 0 | 19 | 35 | **6.B52 dołożył 11 mutacji** (19 → 30): droga zapasowa przez `api.openstreetmap.org` — siatka kafli, czytnik `/api/0.6/map`, tryb `--offline`. Kolumna ocalałych **nieprzeliczona** i taka zostaje: 2 / 2 to pomiar z datą, którego dopisanie kodu nie unieważnia. Przebieg na `7f116a4` (czyste drzewo, `operator,prog`): **22 zabite, 8 ocalałych, 0 nieuruchomionych** — z ośmiu dwie były prawdziwymi dziurami i zostały zamknięte, patrz `reports/osm-api-droga-zapasowa.md` §6. **30 → 31 (09.09.2026, 6.D62)**: wspólna pamięć kafli dołożyła JEDNO porównanie — `if origin == "osm-api" and licznik.pobrane > 1`, czyli warunek przerwy między kaflami, który po zmianie obowiązuje wyłącznie po wyjściu do sieci. Kolumna ocalałych **nieprzeliczona** i taka zostaje, z tego samego powodu co wiersz wyżej. **31 → 35 (12.09.2026, 6.D53)**: czytelnik rejestru źródeł dołożył CZTERY punkty — `w["id"] == "openstreetmap"` i `len(wpisy) != 1` (operator i próg) w `koncowki_osm_z_rejestru`, oraz `z_rejestru == z_kodu` w wypisie par. Wymienione imiennie, bo liczba bez nazw nie pozwala sprawdzić, czy przyrost jest tym, co się dopisało. Kolumna ocalałych **nieprzeliczona** i taka zostaje, z tego samego powodu co dwa wiersze wyżej; zamiast przebiegu te cztery punkty mają kontrole negatywne wykonane wprost (KN-3 i KN-5 w `reports/6d53-dwie-koncowki-osm.md`) |
| `tools/track/crs.py` | wczytywanie | 6 | 4 | -2 | 8 | 14 | `d419437` −2/+3, `16726cd` +1, `f5126a3` (#250) −4 |
| `tools/track/fetch_osm_routes.py` | wczytywanie | 0 | 0 | 0 | 9 | 9 | **09.09.2026 (6.D64) moduł ZMIENIONY, a liczba mutacji stoi** — i to jest zapis, nie przeoczenie: poprzednia wersja tej komórki mówiła, że moduł jest bez zmian od `ff13aa6`, a `git diff` pusty, i to przestało być prawdą. Zmiana zdjęła zmyślony `timestamp_osm_base`, dołożyła dwa pola snapshotu i dwa klucze way’a, czyli same **przypisania**, a `LEGACY_KINDS` liczy `operator, prog` — żadnego nowego porównania nie ma. Kolumna ocalałych **nieprzeliczona** i taka zostaje |
...
| `tools/visual/compare.py` | wizualna | 0 | 0 | 0 | 24 | 26 | **6.D75 dołożył jedno porównanie** (próg luminancji tylnej strony ściany w `backface_fraction`), więc 25 → 26. Ta mutacja jest **ZABITA** — pomiar celowany 09.09.2026: `>` → `<` wywraca `test_klatka_orientacji_odroznia_przod_sciany_od_tylu` (udział przodu wychodzi `1.0` zamiast `0.0`), 2069/2070. Wcześniej: **6.D9 dołożył jedno porównanie** (zgodność sum `IDAT`), więc 24 → 25. Nowa mutacja jest **ZABITA** — pomiar celowany 06.09.2026: `==` → `!=` wywraca trzy testy (`test_te_same_piksele_z_innym_tEXt_sa_zgodne_co_do_bajtu`, `test_jeden_inny_piksel_zapala_bramke_mimo_ze_progi_go_przepuszczaja`, `test_json_wyniku_da_sie_zserializowac`), 1699/1703. Ocalałych nadal zero |
...
Dziś raport zachowuje historyczny pomiar 32 ocalałych; bieżący licznik AST to **108** mutacji. Nie jest to nowy pomiar liczby ocalałych.
