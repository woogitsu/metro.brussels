...
`--view=platform` puts the inspection camera above a generated platform slab when the train reaches a station. It checks the modeled slabs at 6 m on either side of the route axis and uses a side inspection position between stations. This is a camera for reviewing the scene, not a claim about the real boarding side: the technical package currently generates platforms on both sides. [The 1280×720 Parc platform frame](visual-evidence/t400-platform/PARC_platform.png) shows the stopped M7 next to the platform edge, visible side openings, and the Parc marker. [The capture log](visual-evidence/t400-platform/PARC_platform.log) records 54,449 simulation steps, chainage 4075.4 m, speed 0.0 km/h and `widok=Platform`; the HUD reports open doors. PNG SHA-256: `b5d6ae2256a2597ed32c3de7a43d4e3f39e615e8dbf3a89179b510918026f315`.

To reproduce from generated package A assets, run Godot under Xvfb with `--line --limit-kmh=70 --shot=/absolute/path/PARC_platform.png --at-chainage=4075.66 --view=platform`. The frame confirms camera placement and scale in this modeled station only. It does not verify the real side of the Parc platform or the unfinished M7 exterior.
