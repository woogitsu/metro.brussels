...
`tools/track/timetable.py` podczas generowania widzi rzeczywiste `trip_id`, `route_id`, `direction_id`, `block_id` i sekwencję `(stop_id, arrival, departure)` z GTFS. Do `build/timetable.json` zapisuje jednak `duties.rows[].trip_windows` jako same pary `[start, end]`, bez identyfikatora kursu i trasy, `lines[]` jako statystyki taktu, a `segments[]` jako statystyki czasów między stacjami. Sam `build/timetable.json` nie jest wersjonowany i nie leżał lokalnie w chwili audytu. Nawet jego ponowne wygenerowanie w obecnym schemacie nie przywróciłoby relacji okno kursu → kierunek → stacje pakietu A.
...
Następny wymagany input offline to archiwum STIB o **tym samym** `content_sha256`, który stoi w manifeście, lub wersjonowana projekcja jego rekordów dla L1_A, zachowująca wymienione pola i pochodzenie. Po otrzymaniu tych danych można napisać deterministyczny wybór kursów przecinających pakiet A i test porównujący wyjazdy z GTFS. Do tego czasu adapter używający agregatów byłby wymyślonym rozkładem. Nie dodano go, ani polityki dyspozytora.
