...
KATEGORIE = {
    "A": {
        ...
        ("UiTextTests.cs", 1281), ("UiTextTests.cs", 1294), ("UiTextTests.cs", 1312),
        ("SignallingHudTests.cs", 37),
    },
    "B": {
        ("UiTextTests.cs", 1381), ("UiTextTests.cs", 1382),
    },
}
...
def test_regula_po_ksztalcie_literalu_myli_sie_i_dlatego_jej_nie_ma():
    ...
    assert przepuszczone == [("UiTextTests.cs", 1312)], (
    ...
    assert zlapane_z_b == [("UiTextTests.cs", 1382)], (
...
def test_czytnik_widzi_pin_takze_wtedy_gdy_literal_jest_sklejony():
    ...
    assert len(tresci[("UiTextTests.cs", 1281)]) == 122, (
        "sklejanie literałów przestało działać: %d znaków"
        % len(tresci[("UiTextTests.cs", 1281)]))
    assert len(tresci[("UiTextTests.cs", 1312)]) == 98, (
        len(tresci[("UiTextTests.cs", 1312)]))
