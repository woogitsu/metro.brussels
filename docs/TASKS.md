...
| **59 czy 60 stacji** — reguła liczenia | zmierzone 09.09.2026: suma unikalnych przystanków z czterech linii daje **60**, a `network.metro_stations` w `data/network/lines.json` oraz `docs/00-network-data.md` mówią **59**. Najprawdopodobniejsze wyjaśnienie to dwie połowy jednego kompleksu liczone raz, ale **nic w repozytorium tego nie mówi i żadna bramka tych dwóch liczb nie zestawia**. **POMIAR Z 09.09.2026 zawęża to do jednej stacji i OBALA moją własną hipotezę:** oficjalny snapshot GTFS ma **60** unikalnych nazw kanonicznych, `lines.json` też **60** — rozbieżność nie jest więc między dwoma źródłami danych, a między zgodnymi danymi i prozą. Snapshot sam to zapisał i nikt nie zareagował: `"unique_station_names": 60`, `"declared_metro_stations": 59`, `"matches_declared": false`. Liczba `metro_stations: 61` wychodzi z tego, że **Heysel ma dwa rekordy** (peron metra bez rodzica w feedzie). Podsuwana przeze mnie hipoteza, że 59 wychodzi z liczenia Elisabeth i Simonis jako jednego kompleksu, jest **NIEPRAWDZIWA**: GTFS ma je jako dwie osobne stacje, każdą z `routes=['2','6']`. Wyjaśnienia różnicy w danych NIE MA, a proza nie podaje, którą stację wyłącza. Wybór między „proza jest nieaktualna" i „proza stosuje niezapisaną regułę" jest twierdzeniem o sieci, nie wnioskiem z pomiaru. Pomiar: `reports/przystanki-wobec-gtfs.md` |
